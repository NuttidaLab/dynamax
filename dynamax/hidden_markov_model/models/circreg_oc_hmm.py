...
class ParamsOCCircularRegressionHMMEmissions(NamedTuple):
...
class ParamsOCCircularRegressionHMM(NamedTuple):
    initial: ParamsStandardHMMInitialState
    transitions: ParamsStandardHMMTransitions
    emissions: ParamsOCCircularRegressionHMMEmissions


class OCCircularRegressionHMMEmissions(HMMEmissions):
    def __init__(self,
                 num_states,
                 input_dim,
                 emission_dim):
    ...
    def initialize(self,
        ...
        params = ParamsOCCircularRegressionHMMEmissions(
            weights=default(emission_weights, _emission_weights),
            covs=default(emission_covariances, _emission_covs))
        props = ParamsOCCircularRegressionHMMEmissions(
...
class OCCircularRegressionHMM(HMM):
    r"""An HMM whose emissions come by scaling state-dependent weights.
    Origin-Centered. The emission distribution is a von Mises distribution.
    Does not support fitting with Expectation-Maximization.
    ...
    def __init__(self,
        ...
        emission_component = OCCircularRegressionHMMEmissions(num_states, input_dim, emission_dim)
    ...
    def initialize(self,
        ...
        params["emissions"], props["emissions"] = self.emission_component.initialize(key3, method=method, emission_weights=emission_weights, emission_covariances=emission_covariances, emissions=emissions)
        return ParamsOCCircularRegressionHMM(**params), ParamsOCCircularRegressionHMM(**props)
